...
import { isCommercialAircraft } from "@/lib/article-categories";
...
export default async function CommercialAircraftPage() {
  const aircraft = (await listPublicSearchDocuments())
    .filter(isCommercialAircraft)
    ...
    <main className="mx-auto max-w-[1100px] px-5 pb-20 pt-8 sm:px-6">
      ...
      <section className="flex flex-wrap items-end justify-between gap-6">
        <div className="max-w-2xl">
          <Badge
            variant="secondary"
            className="rounded-full text-blue-700 dark:text-blue-300"
          >
            Air transport
          </Badge>
          ...
          <p className="mt-4 text-lg leading-8 text-muted-foreground">
            Explore passenger airliners, regional aircraft, and commercial
            freighters.
          </p>
        </div>
        <Link
      ...
      {aircraft.length ? (
        <section
          ...
          aria-label="Commercial aircraft"
        >
          {aircraft.map((article) => (
            ...
            >
              <Card className="relative h-full gap-0 overflow-hidden py-0 shadow-xs transition-all group-hover:-translate-y-0.5 group-hover:border-blue-500/35 group-hover:shadow-md">
                <ArticleCardBackdrop imageUrl={article.imageUrl} />
                <CardContent className="relative z-10 flex h-full flex-col p-5">
                  <div className="flex items-start justify-between gap-4">
                    <span className="grid size-10 place-items-center rounded-xl bg-blue-500/15 text-blue-700 dark:text-blue-300">
        ...
        <div className="mt-10 rounded-xl border border-dashed p-10 text-center">
          ...
          <p className="mt-3 font-medium">
            No approved commercial aircraft articles yet
          </p>
