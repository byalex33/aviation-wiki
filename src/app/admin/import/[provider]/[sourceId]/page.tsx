...
export default async function ImportPreviewPage({ params, searchParams }: { params: Promise<{ provider: string; sourceId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main>
    ...
    <div className="mt-7 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><strong>No automatic publication.</strong> Importing creates a private draft. It must follow the normal submission, moderator review, and approval workflow.</div>
    <ImportForm>
      ...
      <Card><CardHeader><CardTitle>Verified structured fields</CardTitle></CardHeader><CardContent><div className="space-y-3">{preview.fields.length ? preview.fields.map((field) => <label key={field.id} className="grid cursor-pointer grid-cols-[auto_minmax(0,1fr)] gap-3 rounded-lg border p-3 text-sm sm:grid-cols-[auto_180px_minmax(0,1fr)]"><input type="checkbox" name="fieldId" value={field.id} className="mt-1 size-4"/><strong className="min-w-0 break-words">{field.key}</strong><span className="col-start-2 min-w-0 break-words [overflow-wrap:anywhere] sm:col-start-auto">{field.value}<span className="mt-1 block text-xs text-muted-foreground">Source: {field.sourceLabel} · {field.sourceUrls.length} source {field.sourceUrls.length === 1 ? "page" : "pages"}</span></span></label>) : <p className="text-sm text-muted-foreground">This provider returned no mapped, verifiable fields for the selected content type.</p>}</div>{preview.unverifiedFields.length > 0 && <div className="mt-5 rounded-lg border border-dashed p-4"><p className="flex items-center gap-2 text-sm font-medium"><FileWarning className="size-4 text-amber-600"/>Fields that could not be verified</p><p className="mt-2 text-sm text-muted-foreground">{preview.unverifiedFields.join(", ")}. Missing values are not filled or inferred.</p></div>}</CardContent></Card>
