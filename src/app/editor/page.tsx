...
import { assertArticleEditable, getContributorRestriction } from "@/lib/admin-db";
...
export default async function EditorPage({ searchParams }: { searchParams: Promise<{ type?: string; slug?: string; correction?: string; saved?: string; submitted?: string }> }) {
  ...
  if (article) {
    try {
      assertArticleEditable(article.id, contributor.role, getContributorRestriction(contributor.userId));
    } catch (error) {
