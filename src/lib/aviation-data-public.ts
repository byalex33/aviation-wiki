...
const EMPTY_AVIATION_GRAPH_SNAPSHOT: AviationGraphSnapshot = {
  reconciledAt: null,
  sources: [],
  assertions: [],
  evidence: [],
...
// PostgreSQL "undefined_table" — the additive migration has not been applied in
// this environment yet. Callers behind the AVIATION_DATA_ENABLED flag never hit
// this, but a misconfiguration should degrade to empty pages, not a 500.
function isMissingAviationSchema(error: unknown) {
  return (
    typeof error === "object" &&
    error !== null &&
    "code" in error &&
...
async function readAviationGraphSnapshot(): Promise<AviationGraphSnapshot> {
  const [
    sourceRows,
    assertionRows,
    evidenceRows,
...
export const loadAviationGraphSnapshot = cache(
  async (): Promise<AviationGraphSnapshot> => {
    try {
      return await readAviationGraphSnapshot();
    } catch (error) {
      if (isMissingAviationSchema(error)) {
        console.warn(
          "[aviation-data] graph tables are not present; run `npm run db:migrate:aviation-data -- --apply`. Serving an empty snapshot.",
        );
        return EMPTY_AVIATION_GRAPH_SNAPSHOT;
      }
      throw error;
    }
  },
